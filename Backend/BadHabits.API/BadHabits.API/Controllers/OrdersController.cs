...
<<<<<<< HEAD
        ...
        public IActionResult GetMyOrderHistory()
        {
            ...
            // 2. Kéo toàn bộ hóa đơn và chi tiết hóa đơn về bộ nhớ trước (Chưa có tên áo)
            var orders = _context.Orders
                .Include(o => o.OrderDetails)
                .Where(o => o.UserId == userId)
                .OrderByDescending(o => o.OrderDate)
                .ToList(); // Tải xuống RAM

            // 3. Đóng gói dữ liệu và tự động lục tìm Tên/Ảnh áo cho từng món
            var result = orders.Select(o => new
            {
                ...
                // Lặp qua từng món hàng trong đơn để tìm tên và ảnh
                items = o.OrderDetails.Select(od =>
                {
                    // Lục tìm thông tin áo tương ứng với VariantId
                    var variant = _context.Variants
                                    .Include(v => v.Product)
                    ...
                    return new
                    {
                        productName = variant?.Product?.Name ?? "Sản phẩm C Sủi",
